...
.equ AF7_SLT,         0x700                             // Value to OR into AFRL to select AF7 for PA2 (bits 11:8 = 0111)
...
loop:
			mov r0, #'Y'                                    // Move ASCII 'Y' into r0 (character to transmit)
        	bl  uart_outchar                                // Call transmit routine (blocks until TXE then writes DR)

        	mov r0, #'E'                                    // Move ASCII 'E' into r0
        	bl  uart_outchar                                // Transmit 'E'

        	mov r0, #'S'                                    // Move ASCII 'S' into r0
        	bl  uart_outchar                                // Transmit 'S'
...
uart_init:
	        ...
	        orr r1, r1, #AF7_SLT                            // OR r1 with 0x700 → set bits 11:8 to 0111 (AF7)
